...
ClangResult toMutateAst(const Cursor root, FilesysIO fio, ValidateLoc vloc) @safe {
    ...
    auto rval = ClangResult(mutantAst, visitor.includes.data);
    return rval;
}
...
final class BaseVisitor : ExtendedVisitor {
    ...
    this(FilesysIO fio, ValidateLoc vloc, analyze.Ast* ast) nothrow {
        this.fio = fio;
        this.vloc = vloc;
        this.ast = ast;
    }
    ...
    override bool precondition() scope @safe {
        // this prevents memory exhaustion and stack smash.
        return indent < 150;
    }
    ...
    override void visit(scope const FieldDecl v) @trusted {
        mixin(mixinNodeLog!());
        auto n = ast.make!(analyze.FieldDecl);
        n.schemaBlacklist = isTemplateDecl(v.cursor);

        auto ty = deriveType(*ast, v.cursor.type.canonicalType);
        ty.put(*ast);
        if (ty.type !is null)
            ast.put(n, ty.id);
    ...
    override void visit(scope const CXXFunctionalCastExpr v) @trusted {
        ...
        mixin(mixinNodeLog!());
        auto n = ast.make!(analyze.Poison);
    ...
    override void visit(scope const ClassDecl v) @trusted {
        ...
        auto n = ast.make!(analyze.Poison);
        n.context = true;
        n.schemaBlacklist = n.schemaBlacklist || isParent(CXCursorKind.CXCursor_FunctionTemplate,
    ...
    override void visit(scope const VariableRef v) @trusted {
        ...
        auto n = ast.make!(analyze.VarRef);
        pushStack(n, v);
    ...
    override void visit(scope const DeclRefExpr v) @trusted {
        ...
        n.schemaBlacklist = isParent(CXCursorKind.CXCursor_ClassTemplate,
                ...
                CXCursorKind.CXCursor_FunctionTemplate) != 0;
    ...
    override void visit(scope const IntegerLiteral v) {
        mixin(mixinNodeLog!());
        pushStack(ast.make!(analyze.Literal), v);
        v.accept(this);
    }
    ...
    override void visit(scope const FloatingLiteral v) {
        ...
        pushStack(ast.make!(analyze.FloatLiteral), v);
        v.accept(this);
    }
    ...
    override void visit(scope const ConditionalOperator v) {
        ...
        pushStack(ast.make!(analyze.Poison), v);
        v.accept(this);
    }
    ...
    private bool visitBinaryOp(scope Cursor cursor, ref OperatorCursor op, const CXCursorKind cKind) @trusted {
        ...
        auto blockSchema = op.isOverload || isBlacklist(cursor, op.opLoc)
            || isParent(CXCursorKind.CXCursor_ClassTemplate,
                    CXCursorKind.CXCursor_ClassTemplatePartialSpecialization,
                    CXCursorKind.CXCursor_FunctionTemplate) != 0;
    ...
    private bool visitUnaryOp(scope Cursor cursor, ref OperatorCursor op, CXCursorKind cKind) @trusted {
        ...
        const blockSchema = op.isOverload || isBlacklist(cursor, op.opLoc)
            || isParent(CXCursorKind.CXCursor_ClassTemplate,
                    CXCursorKind.CXCursor_ClassTemplatePartialSpecialization,
                    CXCursorKind.CXCursor_FunctionTemplate) != 0;
    ...
    private auto visitFunc(T)(scope const T v) @trusted {
        ...
        n.schemaBlacklist = v.cursor.kind == CXCursorKind.CXCursor_FunctionTemplate
            || isConstExpr(v);
        nstack.back.children ~= n;
        pushStack(v.cursor, n, loc, v.cursor.kind);
...
enum discreteCategory = AliasSeq!(CXTypeKind.CXType_Char_U,
            CXTypeKind.CXType_UChar, CXTypeKind.CXType_Char16, CXTypeKind.CXType_Char32,
            CXTypeKind.CXType_UShort, CXTypeKind.CXType_UInt, CXTypeKind.CXType_ULong,
            CXTypeKind.CXType_ULongLong, CXTypeKind.CXType_UInt128,
            CXTypeKind.CXType_Char_S, CXTypeKind.CXType_SChar, CXTypeKind.CXType_WChar,
            CXTypeKind.CXType_Short, CXTypeKind.CXType_Int,
            CXTypeKind.CXType_Long, CXTypeKind.CXType_LongLong,
            CXTypeKind.CXType_Int128, CXTypeKind.CXType_Enum);
enum floatCategory = AliasSeq!(CXTypeKind.CXType_Float, CXTypeKind.CXType_Double, CXTypeKind.CXType_LongDouble,
            CXTypeKind.CXType_Float128, CXTypeKind.CXType_Half, CXTypeKind.CXType_Float16);
enum pointerCategory = AliasSeq!(CXTypeKind.CXType_NullPtr, CXTypeKind.CXType_Pointer,
            CXTypeKind.CXType_BlockPointer, CXTypeKind.CXType_MemberPointer,
